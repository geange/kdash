...
func NewCertificateApi(admin *kong.Client) *CertificateApi {
	return &CertificateApi{admin: admin}
}
...
func (s *CertificateApi) Create(ctx *gin.Context) {
	var params kong.Certificate
	_ = ctx.ShouldBindJSON(&params)

	plugin, err := s.admin.Certificates.Create(ctx, &params)
	if err != nil {
...
func (s *CertificateApi) Get(ctx *gin.Context) {
	params := struct {
		ID string `form:"id"`
	}{}
	_ = ctx.ShouldBindQuery(&params)

	item, err := s.admin.Certificates.Get(ctx, &params.ID)
	if err != nil {
...
func (s *CertificateApi) Update(ctx *gin.Context) {
	var params kong.Certificate
	_ = ctx.ShouldBindJSON(&params)

	plugin, err := s.admin.Certificates.Update(ctx, &params)
	if err != nil {
...
func (s *CertificateApi) Delete(ctx *gin.Context) {
	params := struct {
		ID string `json:"id"`
	}{}

	err := s.admin.Certificates.Delete(ctx, &params.ID)
	if err != nil {
...
func (s *CertificateApi) List(ctx *gin.Context) {
	params := struct {
	...
	items, opts, err := s.admin.Certificates.List(ctx, &kong.ListOpt{
		Size:         params.Size,
...
func (s *CertificateApi) ListAll(ctx *gin.Context) {
	items, err := s.admin.Certificates.ListAll(ctx)
	if err != nil {
		global.GVA_LOG.Error(ListError, zap.Any("err", err))
		response.FailWithMessage(ListError, ctx)
		return
